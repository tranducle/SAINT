...
class CERTPreprocessor:
    def __init__(self, dataset_version: str):
        ...
        print(f"Initialized CERT preprocessor for {dataset_version}")
    ...
    def run(self):
        ...
        for user in tqdm(merged['user'].unique()):
            ...
            for i in range(0, len(user_data) - WINDOW_SIZE + 1, STRIDE):
                ...
                window_starts.append(str(w_start))
                window_ends.append(str(w_end))
                
        sequences = np.array(sequences)
        labels = np.array(labels)
        
        # Save
        out_path = OUTPUT_DIR / f"cert_{self.version.replace('.', '')}.pkl"
...
def main():
    ...
    CERTPreprocessor(args.dataset).run()
