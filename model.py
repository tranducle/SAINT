...
Key Components:
- Semantic Multi-Head Attention (SMA) with a grounded first layer
- Temporal Threat Indicator Score (TTIS) from first-layer attention
...
class SemanticMultiHeadAttention(nn.Module):
    ...
    Each head processes one latent slice of the model representation.
    In the first layer, the masked input projection structurally grounds
    each dynamic behavioral modality in its designated head slice while
    shared user-context features remain available to all heads.
    """
    ...
    def forward(
        ...
        # Split H into head-specific latent representations.
        # For the first layer, SAINT.forward has already applied the fixed
        # semantic input mask before this split.
        ...
        for h in range(self.n_heads):
            ...
            # Keep attention in the training graph so diversity and sparsity
            # regularizers can shape the attention patterns. Evaluation is
            # already performed under torch.no_grad(), so a separate detach is
            # unnecessary for explanation export.
...
class SAINT(nn.Module):
    ...
    A transformer-based model with a structurally grounded first semantic
    attention layer and downstream cross-channel fusion for detection.
    """
    ...
    def __init__(
        ...
        # Input embedding for the 30-feature, four-head SAINT configuration.
        # The effective projection is structurally masked so each first-layer
        # head slice receives only its designated dynamic modality plus shared
        # user-context features. Later layers remain free to fuse information.
        self.input_proj = nn.Linear(input_dim, d_model)
        semantic_mask = torch.ones(d_model, input_dim)
        if input_dim == 30 and n_heads == 4:
            semantic_mask.zero_()
            dynamic_groups = [
                list(range(0, 9)),   # Login/File
                list(range(9, 15)),  # Email
                list(range(15, 18)), # Device
                list(range(18, 22)), # Web
            ]
            shared_context = list(range(22, 30))
            for h, dynamic_cols in enumerate(dynamic_groups):
                row_start = h * self.d_head
                row_end = (h + 1) * self.d_head
                allowed = dynamic_cols + shared_context
                semantic_mask[row_start:row_end, allowed] = 1.0
        self.register_buffer('semantic_input_mask', semantic_mask)
    ...
    def forward(
        ...
        # Structurally grounded input embedding. Multiplication by the fixed
        # mask makes cross-dynamic-modality connections exactly zero in the
        # effective first-layer projection.
        effective_weight = self.input_proj.weight * self.semantic_input_mask
        h = F.linear(x, effective_weight, self.input_proj.bias)  # (batch, seq_len, d_model)
    ...
    def compute_ttis(self) -> torch.Tensor:
        ...
        # Use first-layer semantic attention for TTIS. This preserves the
        # structurally grounded modality channels at the explanation source,
        # while deeper layers may fuse information for detection.
        semantic_layer_attn = self.all_attention_weights[0]  # List of (batch, seq, seq)
        beta = self.pool_weights  # (batch, seq)
        
        ttis_scores = []
        for h, A in enumerate(semantic_layer_attn):
...
if __name__ == "__main__":
    ...
    batch_size = 4
    seq_len = 30
    input_dim = 30  # Final paper feature dimension
